...
contract WaveVote is owned {
  ...
  //Struct to save all admin
  address[] public adminAddressList;
  mapping(address => Admin) public adminMap;
  struct Admin {
	  address addr;
	  mapping(address => uint[2]) adminPublicKeyForEachvoterMap;
  }
	 ...
	 */
	  modifier onlyAdmin{
		  assert(adminMap[msg.sender].addr!=address(0x0));
		  _;
	  }
  ...
  struct PeopleToRegister {
      ...
      mapping(address => uint[2]) adminPublicKeyMap;
      mapping(address => uint[2]) adminRegisteredKeyMap;
      uint totalAdminKey;
  }
  ...
  function getPeopleToRegister(address _address) constant public returns (bool _registrationAsked, uint[2] _personalPublicKey, bytes32 _inscriptionCode) {
  ...
  function getVoter(address _address) constant public returns (bool _registered, bool _voteCast, uint[2] _personalPublicKey, uint[2] _registeredkey, uint[2] _reconstructedkey, uint[2] _vote) {
	  ...
	  if (index==0) {
		  ...
		  _registered = voterMap[index].registered;
		  _voteCast = voterMap[index].voteCast;
		  _personalPublicKey = voterMap[index].personalPublicKey;
	      //_adminPublicKey = voterMap[index].adminPublicKey;
	      _registeredkey = voterMap[index].registeredkey;
	      _reconstructedkey = voterMap[index].reconstructedkey;
	      _vote = voterMap[index].vote; 
	  }
  }
  
  //Getter for all boolean of the voter
  function hasAskedForRegistration(address _address) constant public returns(bool) {
	  return peopleToRegisterMap[_address].registrationAsked;
  }
  
  function isRegistered(address _address) constant public returns(bool) {
	  uint index = addressid[_address];
	  return voterMap[index].registered; 
  }
  
  function hasCastVote(address _address) constant public returns(bool) {
	  uint index = addressid[_address];
	  return voterMap[index].voteCast; 
  }
  
  function getTotalAnswers() constant public returns (uint) {
  ...
  function WaveVote() public payable {
    G[0] = Gx;
    G[1] = Gy;
    state = State.SETUP;
  ...
  function sendOneEtherToVoter(address addr) public returns (bool _successful, string _error) {
  ...
  /**
   * Used to add an administrator to the vote
   */
  function addAdmin(address adminAddr) public inState(State.SETUP) onlyOwner returns(bool _successful, string _message) {
	  if(adminMap[adminAddr].addr==address(0x0)) {
		  adminMap[adminAddr].addr = adminAddr;
		  adminAddressList.push(adminAddr);
		  _successful = true;
		  _message = "Admin added";
	  } else {
		  _successful = false;
		  _message = "Admin already added";
	  }
  }
  ...
  function beginSignUp(string _question, bytes32[] _answerListBytes, uint _finishSignupPhase, uint _endSignupPhase, uint _endVotingPhase) public inState(State.SETUP) onlyOwner payable returns (bool){
   ...
   */
  function forceCancelElection() public onlyOwner {
   ...
   */
  function askForRegistration(uint[2] personalPublicKey, bytes32 inscriptionCode) public inState(State.SIGNUP) returns (bool _successful, string _error) {
  ...
  function registerAccount(address accountToRegister, uint[2] adminPublicKey, uint[2] xG, uint[3] vG, uint r) public inState(State.SIGNUP) onlyAdmin returns (bool _successful, string _error) {
	...
	if (!isRegistered(accountToRegister)) {
        if(verifyZKP(xG,r,vG) ) {
        	//Add a new adminPublicKey
        	if(peopleToRegisterMap[accountToRegister].adminPublicKeyMap[msg.sender][0]!=0) {
	           	 _successful = false;
	        	 _error = "You have already register this account";
        	} else {
        		peopleToRegisterMap[accountToRegister].adminPublicKeyMap[msg.sender]=adminPublicKey;
        		peopleToRegisterMap[accountToRegister].adminRegisteredKeyMap[msg.sender]=xG;
        		peopleToRegisterMap[accountToRegister].totalAdminKey+=1;
        		if(peopleToRegisterMap[accountToRegister].totalAdminKey==adminAddressList.length) {
        			uint[2] memory registeredKey;
        			(_successful, _error, registeredKey) = computeRegisteredKey(accountToRegister);
        			if(_successful) {
                        // Update voter's registration
                        addresses.push(accountToRegister);	
                        totalregistered += 1;
                        addressid[accountToRegister] = totalregistered;
                        voterMap[totalregistered] = Voter({
                        	...
                        	reconstructedkey: nullArray,
                        	vote: nullArray});
                        for(uint i=0;i<adminAddressList.length;i++) {
                        	address adminAddr = adminAddressList[i];
                        	voterMap[totalregistered].adminPublicKeyMap[adminAddr] = peopleToRegisterMap[accountToRegister].adminPublicKeyMap[adminAddr];
                        	voterMap[totalregistered].adminRegisteredKeyMap[adminAddr] = peopleToRegisterMap[accountToRegister].adminRegisteredKeyMap[adminAddr];
                        }
                        _error = "Account registered by all admins.";
                        return;
        			} else {
        				return;
        			}
  ...
  function computeRegisteredKey(address accountToRegister) internal constant onlyAdmin returns(bool _successful, string _message, uint[2] _registeredKey) {
	  if(peopleToRegisterMap[accountToRegister].totalAdminKey==adminAddressList.length) {
		  uint[3] memory registeredKey;
		  for(uint i=0;i<adminAddressList.length;i++) {
			  address adminAddress = adminAddressList[i];
			  Secp256k1._addMixedM(registeredKey, peopleToRegisterMap[accountToRegister].adminRegisteredKeyMap[adminAddress]);
			  ECCMath.toZ1(registeredKey,pp);
		  }
		  _registeredKey[0]=registeredKey[0];
		  _registeredKey[1]=registeredKey[1];
		  _successful = true;
		  return;
	  } else {
		  _successful = false;
		  _message = "Not all admin have registered this account";
	  }
  }
   ...
   */
  function computeReconstructedKey(uint indexVoter) public onlyOwner returns(bool _successful, string _message, uint[2] _yG){
      ...
      if (voterMap[indexVoter].reconstructedkey[0]==0) {
      ...
      //@dev see DAT to understand the computation
      ...
      uint[3] memory afteri;
		...
		if (indexVoter==1) {
		      afteri[0] = voterMap[2].registeredkey[0];
		      afteri[1] = voterMap[2].registeredkey[1];
		      afteri[2] = 1;

		      for(uint i=3; i<=totalregistered; i++) {
		         Secp256k1._addMixedM(afteri, voterMap[i].registeredkey);
		      }
			...
			beforei[0] = voterMap[1].registeredkey[0];
			beforei[1] = voterMap[1].registeredkey[1];
			beforei[2] = 1;
           	for(i=2;i<totalregistered;i++) {
           		Secp256k1._addMixedM(beforei, voterMap[i].registeredkey);
        	}
        	...
        	for(uint j=2;j<indexVoter;j++) {
        		Secp256k1._addMixedM(beforei, voterMap[j].registeredkey);
        	}
        	ECCMath.toZ1(beforei,pp);
        	
        	afteri[0] = voterMap[indexVoter+1].registeredkey[0];
        	afteri[1] = voterMap[indexVoter+1].registeredkey[1];
        	afteri[2] = 1;
        	for(j=indexVoter+2;j<=totalregistered;j++) {
        		Secp256k1._addMixedM(afteri, voterMap[j].registeredkey);
        	}
        	ECCMath.toZ1(afteri,pp);
        	afteri[0] = afteri[0];
      ...
      _yG = yG;
      voterMap[indexVoter].reconstructedkey = yG;
      _successful = true;
   ...
   * @dev see the DAT to know what is di, ri, ai and bi
   */
  function submitVote(uint[3] y, uint[2][10] diAndriList, uint[2][10] aList,  uint[2][10] bList) public inState(State.VOTE) returns (bool _successful, string _message) {
     ...
     if(isRegistered(msg.sender) && !hasCastVote(msg.sender)) {
		...
		uint[2] memory xG = voterMap[c].registeredkey;
		uint[2] memory yG = voterMap[c].reconstructedkey;
		...
		if(_successful) {
	    	 voterMap[c].vote[0] = y[0];
	    	 voterMap[c].vote[1] = y[1];

	    	 voterMap[c].voteCast = true;
   ...
   */
  function submitNullVote(address addressToDoNullVote, uint[2] nullVote, uint[3] vG, uint[3] yvG, uint r) public inState(State.VOTE) onlyOwner returns (bool _successful, string _error) {
     // HARD DEADLINE
     ...
     */
     ...
     if(isRegistered(addressToDoNullVote) && !hasCastVote(addressToDoNullVote)) {
       ...
       if(_successful) {
    	 ...
    	 voterMap[c].vote[0] = nullVote[0];
    	 voterMap[c].vote[1] = nullVote[1];
   ...
   */
  function computeSumAllVote() constant public returns(uint[2] _sum) {
	 uint[3] memory temp;
     uint[2] memory vote;
     ...
     for(uint i=1; i<=totalregistered; i++) {
         ...
         assert(voterMap[i].voteCast);
         vote = voterMap[i].vote;
   ...
   */
  function computeTally() inState(State.VOTE) public onlyOwner returns(bool _successful, string _message) {
   ...
   */
  function manualComputeFinalTally(uint result) public onlyOwner returns (bool _successful, string _message){
   ...
   */
  function computeFinalTally(uint result) public onlyOwner returns(bool _successful, string _message){
	if(totalregistered==totalvoted) {
   ...
   */
  function discretLogarithme(uint[2] point) public constant returns (bool _successful, string _message, uint _result){
	  if(point[0] == 0) {
	       _successful = false;
  ...
  function verifyZKP(uint[2] xG, uint r, uint[3] vG) internal constant returns (bool){
  ...
  function verifyZKPNullVote(uint[2] xG, uint[2] yG, uint[2] voteNull, uint r, uint[3] vG, uint[3] vH) public constant returns (bool _successful, string _error){
   ...
   */
	function verifyZKPVote(uint[3] y, uint[2][2] res2D, uint[2][10] diAndriList, uint[2][10] aList,  uint[2][10] bList) internal constant returns (bool _successful, string _message) {
