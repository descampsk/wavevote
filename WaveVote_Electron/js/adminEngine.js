...
function sendRegistrationAccepted(doc, question) {
	...
	var mailOptions = {
			  ...
			  html: "Bonjour " + name + " " + lastName + " !<br><br>" +
			  		"L'administrateur a accepté votre demande d'inscription pour le vote « " + question + " ».<br>" +
	...
	transporter.sendMail(mailOptions, function(error, info){
		  if (error) {
			...
			db.update({ _id: doc._id }, { $set: { RegistrationAcceptedMail: true } }, {});
		    console.log('Email sent: ' + info.response);
...
function sendRappelRegistration(doc, question) {
	...
	var mailOptions = {
			  ...
			  html: "Bonjour " + name + " " + lastName + " !<br><br>" +
			  		"Vous n'avez pas encore envoyé votre demande d'inscription pour le vote « " + question + " ».<br>" +
...
function sendAllRegistrationAccepted(question) {
	db.find({RegistrationAcceptedMail: { $exists: false }, registered: true}, function(error,docs) {
		if(error) {
			console.log(error);
		} else {
...
function generateEachInscriptionKey() {
	var lineReader = require('readline').createInterface({
		...
		lineReader.on('line', function (line) {
			...
			db.insert({numero: numero, name: name, lastName: lastName, mail: line, registered: false});
			numero+=1;
		});
}